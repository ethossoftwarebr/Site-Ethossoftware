...
- **DO** add new bundled-into-prod npm packages to the `allowlist` in `scripts/build.ts:7-33`. Anything not in the allowlist stays external and must be installed at runtime.
...
- **DON'T** use top-level `await` in `server/index.ts`. The IIFE at `:62` exists because esbuild emits CJS (`scripts/build.ts:53`) where TLA breaks.
